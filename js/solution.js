// Главный файл, содержит важные переменные, объединяет остальные
...
// перемещается, если тянем за "корешок"
...
function removeForm() {
  Array.from(document.querySelectorAll('.comments__form')).forEach(comment => {
    comment.remove()
  });
}
